...
(function () {
  ...
  function wrapZoom(pre) {
    ...
    // Дефолт: схема вписана ПО ШИРИНЕ (текст крупный и читаемый сразу, без действий).
    // Высоту НЕ ограничиваем жёстко — для длинных вертикальных (TB) схем
    // естественна обычная прокрутка страницы, а не сжатие в нечитаемую колонку.
    // Исключение: если при ширине 100% схема получается экстремально высокой
    // (узкая вертикальная диаграмма), отдаём приоритет высоте, чтобы она влезла
    // в экран — текст останется читаемым, т.к. узлов в таких схемах мало.
    var svg = pre.querySelector('svg');
    if (svg) {
      svg.removeAttribute('width');
      svg.removeAttribute('height');
      svg.style.height = 'auto';
      // По умолчанию — по ширине: текст узлов всегда крупный и читаемый.
      // Длинная схема просто продолжается вниз, читатель скроллит страницу
      // (привычное поведение, НЕ «лишнее действие»).
      svg.style.width = '100%';
